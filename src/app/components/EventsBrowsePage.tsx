...
import { fetchEvents, fetchLikedEvents, resolveImageUrl } from "@/app/lib/api";
...
export default function EventsBrowsePage({ likedOnly = false }: { likedOnly?: boolean }) {
  const {t} = useUI();
  ...
  const [activePreset, setActivePreset] = useState<"upcoming" | "1d" | "7d" | "1m" | "past" | "all" | "">("upcoming");

  const [sortOrder, setSortOrder] = useState<"asc" | "desc">(likedOnly ? "desc" : "asc");

  const handleDatePreset = (preset: "upcoming" | "1d" | "7d" | "1m" | "past" | "all") => {
    ...
    if (preset === "past") {
      ...
      setDateFrom("");
      setDateTo(getLocalDateString(yesterday));
      setSortOrder("desc");
      return;
    }
    ...
    setSortOrder(likedOnly ? "desc" : "asc");
  ...
  // Ignore responses from earlier filters/pages, including after unmount.
    ...
    const timer = setTimeout(async () => {
      try {
        const filters: IEventFilters = {
          ...
          sort_order: likedOnly ? "desc" : sortOrder,
          page,
          pageSize: 12,
        };
        const res = await (likedOnly ? fetchLikedEvents(filters) : fetchEvents(filters));
        if (cancelled) return;
        if (page > Math.max(1, res.pagination.totalPages)) {
          setPage(Math.max(1, res.pagination.totalPages));
          return;
        }
        setEvents(res.data);
  ...
  }, [search, locationType, dateFrom, dateTo, page, sortOrder, likedOnly, retry]);
  ...
  <div className="flex flex-wrap items-center gap-1.5 max-w-full sm:pl-4 sm:border-l border-gray-200 dark:border-gray-700 vibrant:border-purple-200">
    {(likedOnly ? [
      ...
      <button
        key={preset.id}
        onClick={() => handleDatePreset(preset.id as Parameters<typeof handleDatePreset>[0])}
