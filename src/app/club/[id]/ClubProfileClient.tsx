...
import { updateClub, uploadImage } from '@/app/lib/api'; // Ensure updateClub is exported from api.ts
...
export default function ClubProfileClient({ initialClub, events }: { initialClub: any, events: any[] }) {
  ...
  // Is there a user? AND Do the IDs match?
  ...
  // Form State (initialized with club data)
  const [formData, setFormData] = useState({
    ...
    // Add other fields like website if/when your DB supports them
  });
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        if (response.success) {
            setClub(response.data); // Update local UI with server response
            setIsEditing(false);
        }
    } catch (error) {
  ...
  const handleCancel = () => {
    // Reset form to current club data
    setFormData({
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-20">
      ...
      <div className="bg-white shadow-sm mb-6">
        ...
        {/* BANNER AREA */}
        <div className="relative h-48 md:h-64 w-full bg-gray-900 overflow-hidden group">
          {formData.banner_url ? (
            <img 
              src={formData.banner_url} 
              alt="Club Banner" 
              className={`w-full h-full object-cover transition-opacity ${isEditing ? 'opacity-70' : 'opacity-90'}`}
            />
          ) : (
          ...
          {isEditing && (
            <label className="absolute inset-0 flex items-center justify-center cursor-pointer bg-black/30 hover:bg-black/40 transition-colors z-20">
                <div className="bg-white/90 backdrop-blur text-gray-900 px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-end -mt-12 md:-mt-16 mb-6 gap-6 relative z-10">
            ...
            {/* LOGO AREA */}
            <div className="bg-white p-1.5 rounded-2xl shadow-lg shrink-0 relative group">
              <img 
              ...
              {isEditing && (
                <label className="absolute inset-0 flex items-center justify-center cursor-pointer rounded-2xl bg-black/30 hover:bg-black/40 transition-colors z-20">
                ...
                <div className="flex-1">
                    ...
                    {isEditing ? (
                        <input 
                            ...
                            className="text-3xl md:text-4xl font-bold text-gray-900 border-b-2 border-blue-500 focus:outline-none bg-transparent w-full"
                        />
                    ) : (
                    ...
                    {isEditing ? (
                        <>
                            <button 
                                ...
                                disabled={isSaving}
                                className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold transition-all hover:bg-gray-50"
                            >
                                <X className="w-4 h-4" />
                                Cancel
                            </button>
                        ...
                        <>
                            ...
                            {isOwner && (
                                <button 
                                    ...
                                    className="flex items-center gap-2 bg-blue-50 hover:bg-blue-100 text-blue-700 px-4 py-2.5 rounded-xl font-bold transition-colors mr-2"
                                >
                            ...
                            <Link 
                                ...
                                className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-4 py-2.5 rounded-xl font-bold transition-all hover:bg-gray-50 hover:border-gray-300 ml-2"
                            >
                                <Plus className="w-4 h-4" />
                                <span className="hidden sm:inline">Post Event</span>
                            </Link>
                            <a href={`mailto:${club.email}`} className="flex items-center gap-2 bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-sm ml-2">
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100 relative">
                ...
                {isEditing ? (
                    <textarea 
                        ...
                        className="w-full h-48 p-4 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg leading-relaxed text-gray-700"
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="w-full">
                            ...
                            {isEditing ? (
                                ...
                                <a href={`mailto:${club.email}`} className="text-sm font-semibold text-gray-900 hover:text-blue-600 break-all">{club.email}</a>
                            )}
                        </div>
                    </div>
