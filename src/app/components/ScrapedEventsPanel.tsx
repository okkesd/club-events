...
import {
    getScrapedEvents, approveScrapedEvent, rejectScrapedEvent,
    deleteScrapedEvent, updateScrapedEvent, importScrapedEvents,
    getAdminClubs, resolveImageUrl, getIgClubMappings, deleteIgClubMapping,
} from "@/app/lib/api";
import {
    IScrapedEvent, IScrapedEventApprove, ScrapedEventStatus,
    ClubData, Pagination, IIgClubMapping,
} from "@/app/lib/types";
...
export default function ScrapedEventsPanel({ onPendingCountChange }: { onPendingCountChange?: (n: number) => void }) {
    ...
    }, [status, clubFilter, page, onPendingCountChange]);
    ...
    useEffect(() => { setPage(1); }, [status, clubFilter]);
    ...
    const handleApproved = (row: IScrapedEvent, eventId?: string) => {
        removeRow(row.id);
        setSelected(null);
        setBanner({
            text: `"${row.title || "Candidate"}" published.`,
            href: eventId ? `/event/${eventId}` : undefined,
        });
        fetchRows();
    };
        ...
        <div>
            ...
            {banner && (
                <div className="m-5 mb-0 p-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 flex items-center justify-between gap-3">
                    <p className="text-sm font-medium text-green-700 dark:text-green-300">
                        {banner.text}{" "}
                        {banner.href && (
                            <a href={banner.href} target="_blank" className="underline font-bold">View event</a>
                        )}
                    </p>
            ...
            {isLoading ? (
                ...
                <div className="p-12 text-center text-gray-400 dark:text-gray-600">
                    ...
                    <p>No {status === "all" ? "" : status} candidates.</p>
                </div>
            ) : (
...
function ReviewModal({
    ...
    clubs: ClubData[];
    onClose: () => void;
    onApproved: (row: IScrapedEvent, eventId?: string) => void;
    ...
    const handleApprove = async () => {
        ...
        try {
            const res = await approveScrapedEvent(row.id, {
                ...form,
                clubId: form.publishAsAdmin ? undefined : form.clubId,
                ...
                coverImage: isCoverOverridden ? form.coverImage : undefined,
            });
            onApproved(row, res.data?.id);
        } catch (err: any) {
